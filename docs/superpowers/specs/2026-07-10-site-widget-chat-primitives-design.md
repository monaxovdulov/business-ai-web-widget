...
Статус: одобрено пользователем, ожидает письменного ревью
Дата: 2026-07-10
...
AI — необязательный безопасный первичный ассистент, работающий только внутри `granit-operations` после сохранения входящего сообщения. Создание лида не зависит от доступности AI.

AI может:

- быстро ответить на первое сообщение;
- собрать недостающие детали и задать максимум один уточняющий вопрос;
- объяснить общие вещи про памятники, материалы, оформление и этапы;
- поддержать первый контакт до подключения менеджера.
